...
bigTest = "[x y z] 32 * (x * y * z) + (y / (33+2*(x*y)))"

dumpProgram :: String -> IO ()
dumpProgram prog = do
  ast <- return $ pass2 . pass1 $ prog
  asm <- return $ compile prog
  putStrLn "================="
  putStrLn prog
  putStrLn $ "---\nAST: " ++ show ast
  putStrLn "---\nASM:"
...
main :: IO ()
main  = do 
  dumpProgram ex1_prog
  dumpProgram ex2_prog
  dumpProgram ex3_prog
  dumpProgram bigTest
  putStrLn ""
  putStrLn ""
